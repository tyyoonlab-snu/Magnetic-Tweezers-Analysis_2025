function [LL, prior, transmat, mu, Sigma, mixmat, mu_history] = learn_mhmm(data, prior, transmat, mu, Sigma, mixmat,max_iter, thresh, verbose, cov_type, static)
...
if ~exist('max_iter'), max_iter = 10; end
if ~exist('thresh'), thresh = 1e-4; end
if ~exist('verbose'), verbose = 1; end
if ~exist('cov_type'), cov_type = 'full'; end
if ~exist('static'), static = 0; end
...
for ex=1:numex
    ...
    [alpha, beta, gamma, xit, current_loglik, gamma2] = forwards_backwards(prior, transmat, B, B2, mixmat);
